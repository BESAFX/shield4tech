app.controller("menuCtrl", [
    ...
    function ($scope,
        ...
        $scope.rowMenuCustomers = [
            ...
            {
                html: '<div class="drop-menu">' +
                '<img src="/ui/img/enable.png" width="24" height="24">' +
                '<span>' + ($rootScope.lang === 'AR' ? 'تفعيل...' : 'Enable...') + '</span>' +
                '</div>',
                enabled: function () {
            ...
            {
                html: '<div class="drop-menu">' +
                '<img src="/ui/img/disable.png" width="24" height="24">' +
                '<span>' + ($rootScope.lang === 'AR' ? 'تعطيل...' : 'Disable...') + '</span>' +
                '</div>',
                enabled: function () {
            ...
            {
                html: '<div class="drop-menu">' +
                '<img src="/ui/img/about.png" width="24" height="24">' +
                '<span>' + ($rootScope.lang === 'AR' ? 'التفاصيل...' : 'Details...') + '</span>' +
                '</div>',
                enabled: function () {
                    return true;
                },
        ...
        $scope.deleteProductCategory = function (team) {
            ModalProvider.openConfirmModel(
                ...
                "stars",
                $rootScope.lang === 'AR' ? "هل تود حذف التصنيف فعلاً؟" : "Are your really want to delete this item ?").result.then(function (data) {
                if (data) {
        ...
        $scope.deleteProduct = function (team) {
            ModalProvider.openConfirmModel(
                $rootScope.lang === 'AR' ? "الاصناف" : "Categories",
                "stars",
                $rootScope.lang === 'AR' ? "هل تود حذف الصنف فعلاً؟" : "Are your really want to delete this item ?")
                .result.then(function (data) {
                if (data) {
                    ProductService.remove(team.id).then(function () {
                        var index = $scope.products.indexOf(team);
        ...
        $scope.rowMenuProduct = [
            ...
            },
            {
                ...
                click: function ($itemScope, $event, value) {
                    $scope.deleteProduct($itemScope.product);
                }
            }
        ];
        $scope.checkAllProducts = function () {
            ...
            for (var i = 0, n = elements.length; i < n; i++) {
                var element = elements[i];
                if ($('#checkAllProducts input').is(":checked")) {
                    element.MaterialCheckbox.check();
                }
                else {
                    element.MaterialCheckbox.uncheck();
            ...
            angular.forEach($scope.products, function (product) {
                product.isSelected = $scope.products.checkAll;
            });
        };
        $scope.checkProduct = function () {
            ...
            for (var i = 0, n = elements.length; i < n; i++) {
                var element = elements[i];
                if ($('.check input:checked').length == $('.check input').length) {
                    document.querySelector('#checkAllProducts').MaterialCheckbox.check();
                } else {
                    document.querySelector('#checkAllProducts').MaterialCheckbox.uncheck();
                }
            }
        };
