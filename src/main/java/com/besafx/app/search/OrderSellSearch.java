...
import com.besafx.app.service.sell.OrderSellService;
...
public class OrderSellSearch {
    ...
    private final Logger log = LoggerFactory.getLogger(OrderSellSearch.class);

    @Autowired
    private OrderSellService orderService;

    public List<OrderSell> filter(
            final Long codeFrom,
            final Long codeTo,
            final Long dateFrom,
            final Long dateTo
        ...
        if (!predicates.isEmpty()) {
            ...
            list.sort(Comparator.comparing(OrderSell::getCode).reversed());
            return list;
        } else {
            List<OrderSell> list = Lists.newArrayList(orderService.findAll());
            list.sort(Comparator.comparing(OrderSell::getCode).reversed());
            return list;
        }
    }

    public List<OrderSell> findByToday() {
        List<Specification> predicates = new ArrayList<>();
    ...
    public List<OrderSell> findByWeek() {
        List<Specification> predicates = new ArrayList<>();
    ...
    public List<OrderSell> findByMonth() {
        List<Specification> predicates = new ArrayList<>();
    ...
    public List<OrderSell> findByYear() {
        List<Specification> predicates = new ArrayList<>();
